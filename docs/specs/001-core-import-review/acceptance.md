...
| 购物神评 AB 语义 | 已确认 | `HY-Vision-2.0-instruct`=A=优化后 Prompt；`quinta_gouwushenping_firstround`=B=优化前 Prompt；以工作表列标识为准 |
...
- 状态：通过
- 分支：`feat/merge-001-exact-merge`
- 验证命令：`/Users/huagnqinlin/.workbuddy/binaries/python/versions/3.13.12/bin/python3 -W error::ResourceWarning -m unittest discover -s tests -v`
- 结果：15 项测试通过，未出现资源警告。
- 已实现：`POST /api/v1/import-batches/{id}/merge`、`GET /api/v1/import-batches/{id}`、按 `case_id + side + evaluation_version` 精确分组、人工/LLM/机检跨文件合并、覆盖分布、来源引用、文本/同评分方数值冲突报告、原子替换对齐结果、`ALIGNED`/`PARTIAL_FAILURE` 状态更新。
- 已知限制：当前只生成 `aligned_cases`，尚未应用四类规则、生成 badcase 候选或入队企微投影；这些属于 `RULE-001` 与 `WECOM-001`。

## 购物神评 AB 适配器验收记录

- 状态：通过
- 真实输入：`/Users/huagnqinlin/Downloads/购物神评首轮hy_vs_claude_v3_106 (2).xlsx`（仅在临时 SQLite/工件目录中读取与验证，未写企微、未提交数据文件）。
- 验收结果：`详细配对结果` 读取 149 行；解析 149 个配对单元；拆出 298 条对齐记录；A 侧 149 条、B 侧 149 条；全部为 `LLM` 覆盖；无冲突或无效行。
